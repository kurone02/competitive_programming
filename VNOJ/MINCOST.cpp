...
public:
    ...
    inline void AugmentFlow(){
        ...
        for(int u=sink,i;(i=trace[u])>=0;u=e[i].x)
            delta=min(delta,e[i].flow<0? -e[i].flow : e[i].cap);
        for(int u=sink,i;(i=trace[u])>=0;u=e[i].x){
            e[i].cap-=delta;
            e[i^1].cap+=delta;
            e[i].flow+=delta;
...
int main(){
    ...
    for(int u=1;u<=n;u++)
        for(int v=1;v<=n;v++) if(d[u][v]>0)
            edmond.AddEdge(u,v,d[u][v],c[u][v]);
    if(edmond.MaxFlow()!=k) return cout<<-1,0;
    cout<<edmond.MinCost()<<'\n';
    int totalCost=0;
    for(int i=2;i<edmond.e.size();i+=2){
        int flow=max(edmond.e[i].flow,edmond.e[i^1].flow);
        if(flow==0) continue;
        cout<<edmond.e[i].x<<' '<<edmond.e[i].y<<' ' <<flow<<'\n';
    }
    cout<<"0 0 0";
